...
- Clases principales:
  ...
  - `CatalogService` — Capa de negocio del catálogo. `obtenerTodos(Pageable)` retorna `Page<LibroResponseDTO>` usando `findAll(pageable).map()`.
- Endpoints expuestos:
...
- Cobertura de tests: ~95% (158 tests, 0 fallos, 0 errores)
